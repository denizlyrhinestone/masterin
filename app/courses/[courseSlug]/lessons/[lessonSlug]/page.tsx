...
export async function generateMetadata({ params }: LessonPageProps): Promise<Metadata> {
  const { course, lesson } = await getLessonBySlug(params.courseSlug, params.lessonSlug)
...
export default async function LessonPage({ params }: LessonPageProps) {
  ...
  const { course, lesson, progress, previousLesson, nextLesson } = await getLessonBySlug(
    params.courseSlug,
    params.lessonSlug,
    userId,
  )
